...
export default function App(){
  ...
  useEffect(()=>{
    const before=e=>{e.preventDefault();setInstallPrompt(e)},installed=()=>setInstallPrompt(null)
    addEventListener('beforeinstallprompt',before);addEventListener('appinstalled',installed)
    return()=>{removeEventListener('beforeinstallprompt',before);removeEventListener('appinstalled',installed)}
  },[])

  useEffect(()=>{
    if(!('serviceWorker'in navigator))return
    let cancelled=false
    const key='tripos-map-anchors-z11-v1'
    const prefetch=async()=>{
      if(cancelled||!navigator.onLine||localStorage.getItem(key))return
      try{
        ...
        const result=await new Promise(resolve=>{
          ...
          target.postMessage({type:'PREFETCH_MAP_ANCHORS',zoom:11,coordinates:data.places.filter(p=>Number.isFinite(p.latitude)&&Number.isFinite(p.longitude)).map(p=>[p.longitude,p.latitude])},[channel.port2])
        })
        if(result?.ok&&!cancelled)localStorage.setItem(key,new Date().toISOString())
      }catch{}
    }
