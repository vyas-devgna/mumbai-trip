...
export default function MapScreen({setDay}){
  ...
  return <section className="map-page"><div className="map-title"><div className="page-title"><span>GEOGRAPHIC BOARD</span><h1>Map</h1></div><div className="segment"><button className={mode==='satellite'?'active':''} onClick={()=>setMode('satellite')}>Satellite</button><button className={mode==='street'?'active':''} onClick={()=>setMode('street')}>Street</button></div></div><div className="map-days"><button className={scope==='mumbai'?'active':''} onClick={()=>setScope('mumbai')}>Mumbai</button>{DAYS.map(d=><button key={d} className={scope===d?'active':''} onClick={()=>{setScope(d);setDay(d)}}>{new Date(`${d}T12:00:00+05:30`).getDate()}</button>)}</div><div className="map-layout"><div className="map-frame"><div ref={el} className="map-canvas"/><div className="map-legend"><span><i/> planned sequence</span><span>anchor tiles prefetched · live detail online</span></div></div><article className="panel location-panel"><div className="panel-head"><span>TRIP LOCATIONS</span><b>{data.places.length} ANCHORS</b></div><div className="location-list">{data.places.map((p,i)=><a key={p.id} href={mapsUrl(p)} target="_blank" rel="noreferrer"><span className="location-index">{String(i+1).padStart(2,'0')}</span><span className="location-copy"><b>{p.name}</b><small>{p.address||p.category}</small></span><ExternalLink aria-hidden="true"/></a>)}</div></article></div></section>
}
