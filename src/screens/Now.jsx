...
export default function Now({now,expenses,notes,setResource,setSheet,setTab}){
  ...
  return <section className="dash">
    <article className="panel mission span2">
      <div className="panel-head"><span>01 / NEXT ANCHOR</span><b>{next?.timing.type}</b></div>
      <div className="mission-grid"><div><p className="stamp">{fmtDate(next.date)} · {timingLabel(next)}</p><h1>{next.title}</h1><p>{next.notes?.[0]}</p></div><div className="count"><strong>{hours>48?Math.ceil(hours/24):hours}</strong><span>{hours>48?'days':'hours'} to outbound</span></div></div>
      <div className="actions">{place?.googleMapsUrl&&<a className="primary" href={place.googleMapsUrl} target="_blank" rel="noreferrer">Navigate</a>}<button onClick={()=>setTab('Plan')}>Open plan</button>{next.sourceIds?.[0]&&<button onClick={()=>setResource(byId(data.resources,next.sourceIds[0]))}>Open file</button>}</div>
    </article>
    ...
    <article className="panel alerts"><div className="panel-head"><span>03 / SIGNALS</span><b>{data.signals.length} OPEN</b></div>{data.signals.map(s=><div className="alert" key={s.id}><i>!</i><div><b>{s.label}</b><small>{s.status.replaceAll('-',' ')}</small></div></div>)}</article>

    <article className="panel span2"><div className="panel-head"><span>04 / FIXED SPINE</span><b>DON'T BREAK THESE</b></div><div className="anchor-track">{fixed.map((a,i)=>{const r=a.sourceIds?.[0]&&byId(data.resources,a.sourceIds[0]);return <button className="anchor" key={a.id} onClick={()=>r&&setResource(r)}><em>0{i+1}</em><b>{fmtShortDate(a.date)} · {a.timing.start}</b><strong>{a.title}</strong><small>{a.placeId?byId(data.places,a.placeId)?.name:'—'}</small></button>})}</div></article>
