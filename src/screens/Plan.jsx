...
function Item({a,i,setResource}){
  ...
  return <article className={`timeline-item ${a.timing.type}`}><div className="timeline-time"><b>{timingLabel(a)}</b><span>{a.timing.type}</span></div><div className="timeline-line"><i/></div><div className="timeline-content"><div className="timeline-title"><div><small>0{i+1} · {a.priority}</small><h3>{a.title}</h3></div><em>{a.status}</em></div>{p&&<p>{p.name}</p>}<p className="muted">{a.notes?.[0]}</p><div className="people-chips">{a.participants.map(id=><span key={id}>{byId(data.members,id)?.initials}</span>)}{r&&<button onClick={()=>setResource({...r,boarding:true})}>Open file ↗</button>}</div></div></article>
}
